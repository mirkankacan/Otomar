...
public class PaymentServiceTests
{
    ...
    [Fact]
    public async Task GetPaymentByIdAsync_Found_ReturnsOk()
    {
        // Arrange
        var paymentId = Guid.NewGuid();
        var payment = new PaymentDto { Id = paymentId, OrderCode = "OTOMAR-001", TotalAmount = 500m, Status = PaymentStatus.Completed };
        ...
        result.Data.OrderCode.Should().Be("OTOMAR-001");
    }

    [Fact]
    public async Task GetPaymentByIdAsync_NotFound_ReturnsNotFound()
    {
        // Arrange
        var paymentId = Guid.NewGuid();
        _paymentRepositoryMock.Setup(x => x.GetByIdAsync(paymentId)).ReturnsAsync((PaymentDto?)null);
    ...
    #region GetPaymentByOrderCodeAsync - Happy Path Tests

    [Fact]
    public async Task GetPaymentByOrderCodeAsync_Found_ReturnsOk()
    {
        // Arrange
        var payment = new PaymentDto { Id = Guid.NewGuid(), OrderCode = "OTOMAR-001", TotalAmount = 500m };
        _paymentRepositoryMock.Setup(x => x.GetByOrderCodeAsync("OTOMAR-001")).ReturnsAsync(payment);

        // Act
        var result = await _sut.GetPaymentByOrderCodeAsync("OTOMAR-001");
    ...
    [Fact]
    public async Task GetPaymentByOrderCodeAsync_NotFound_ReturnsNotFound()
    {
        // Arrange
        _paymentRepositoryMock.Setup(x => x.GetByOrderCodeAsync("OTOMAR-999")).ReturnsAsync((PaymentDto?)null);

        // Act
        var result = await _sut.GetPaymentByOrderCodeAsync("OTOMAR-999");
    ...
    [Fact]
    public async Task GetPaymentsAsync_HasPayments_ReturnsOk()
    {
        // Arrange
        var payments = new List<PaymentDto>
        {
            new() { Id = Guid.NewGuid(), OrderCode = "OTOMAR-001", TotalAmount = 500m }
        };
        _paymentRepositoryMock.Setup(x => x.GetAllAsync()).ReturnsAsync(payments);
    ...
    [Fact]
    public async Task GetPaymentsAsync_Empty_ReturnsNotFound()
    {
        // Arrange
        _paymentRepositoryMock.Setup(x => x.GetAllAsync()).ReturnsAsync(Enumerable.Empty<PaymentDto>());
    ...
    #region GetPaymentsByUserAsync - Happy Path Tests

    [Fact]
    public async Task GetPaymentsByUserAsync_HasPayments_ReturnsOk()
    {
        // Arrange
        var payments = new List<PaymentDto>
        {
            new() { Id = Guid.NewGuid(), OrderCode = "OTOMAR-001", UserId = "user-1" }
        };
        _paymentRepositoryMock.Setup(x => x.GetByUserAsync("user-1")).ReturnsAsync(payments);

        // Act
        var result = await _sut.GetPaymentsByUserAsync("user-1");
    ...
    [Fact]
    public async Task GetPaymentsByUserAsync_Empty_ReturnsNotFound()
    {
        // Arrange
        _paymentRepositoryMock.Setup(x => x.GetByUserAsync("user-1")).ReturnsAsync(Enumerable.Empty<PaymentDto>());
