...
namespace Otomar.Persistence.Repositories
{
    ...
    /// </summary>
    public class ListSearchRepository : IListSearchRepository
    {
        private readonly IUnitOfWork _unitOfWork;

        public ListSearchRepository(IUnitOfWork unitOfWork)
        {
        ...
        public async Task<(string? RequestNo, string? CreatedBy)?> GetListSearchCreatorInfoAsync(Guid listSearchId)
        {
            ...
            var row = await _unitOfWork.Connection.QueryFirstOrDefaultAsync<dynamic>(query, new { Id = listSearchId });
        ...
        public async Task<ListSearchDto?> GetByIdAsync(Guid id)
        {
            var listSearchQuery = @"
                ...
                WHERE Id = @id;";
            var listSearchRow = await _unitOfWork.Connection.QueryFirstOrDefaultAsync<dynamic>(listSearchQuery, new { id });
        ...
        public async Task<ListSearchDto?> GetByRequestNoAsync(string requestNo)
        {
            ...
            var listSearchRow = await _unitOfWork.Connection.QueryFirstOrDefaultAsync<dynamic>(listSearchQuery, new { requestNo });
        ...
        public async Task<IEnumerable<ListSearchDto>> GetPagedAsync(int offset, int pageSize)
        {
            ...
            var listSearches = await _unitOfWork.Connection.QueryAsync<dynamic>(listSearchQuery, new { offset, pageSize });
        ...
        public async Task<IEnumerable<ListSearchDto>> GetAllAsync()
        {
            ...
            var listSearches = await _unitOfWork.Connection.QueryAsync<dynamic>(listSearchQuery);
            var listSearchList = listSearches.ToList();
        ...
        public async Task<IEnumerable<ListSearchDto>> GetByUserAsync(string userId)
        {
            ...
            var listSearches = await _unitOfWork.Connection.QueryAsync<dynamic>(listSearchQuery, new { userId });
            var listSearchList = listSearches.ToList();
        ...
        private async Task<ListSearchDto> MapListSearchWithPartsAndAnswersAsync(dynamic listSearchRow)
        {
            ...
            var parts = await _unitOfWork.Connection.QueryAsync<dynamic>(partsQuery, new { ListSearchId = listSearchId });
            ...
            var answersQuery = @"
                ...
                WHERE ListSearchId = @ListSearchId;";
            var answers = await _unitOfWork.Connection.QueryAsync<dynamic>(answersQuery, new { ListSearchId = listSearchId });
        ...
        private async Task<IEnumerable<ListSearchDto>> MapListSearchListWithPartsAndAnswersAsync(List<dynamic> listSearchList)
        {
            ...
            var parts = await _unitOfWork.Connection.QueryAsync<dynamic>(partsQuery, new { ListSearchIds = listSearchIds });
            var partsList = parts.ToList();
            ...
            var answersQuery = @"
                ...
                FROM IdtListSearchAnswers WITH (NOLOCK)
                WHERE ListSearchId IN @ListSearchIds;";
            var answers = await _unitOfWork.Connection.QueryAsync<dynamic>(answersQuery, new { ListSearchIds = listSearchIds });
            var answersDict = answers.ToDictionary(a => (int)a.ListSearchPartId);
