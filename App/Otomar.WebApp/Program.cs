...
app.UseStaticFiles(new StaticFileOptions
{
    OnPrepareResponse = ctx =>
    {
        ...
        // Cache herkese uygulanır
        headers[Microsoft.Net.Http.Headers.HeaderNames.CacheControl] =
            "public,max-age=" + durationInSeconds;

        // SADECE wwwroot/assets/img/cars klasoru
        var carsFolder = Path.Combine("wwwroot", "assets", "img", "cars");

        var physicalPath = ctx.File.PhysicalPath;

        if (physicalPath != null &&
            physicalPath.Contains(carsFolder, StringComparison.OrdinalIgnoreCase))
        {
