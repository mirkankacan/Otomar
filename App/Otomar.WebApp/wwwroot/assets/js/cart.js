...
const CartManager = {
    ...
    renderCartPage(cart) {
        ...
        items.forEach(item => {
            ...
            const isOverride = !!(item.listSearchAnswerId ?? item.ListSearchAnswerId);
            const lineTotal = unitPrice * quantity;
            const priceStr = this.formatPrice(unitPrice).replace(/^₺| ₺$/g, '').trim();
            const lineTotalStr = this.formatPrice(lineTotal).replace(/^₺| ₺$/g, '').trim();
            const slug = generateSlug(productName);

            const priceLabelHtml = isOverride
                ? `<span class="cart__price">₺${priceStr}</span><small style="display:block;color:var(--secondary-color);font-size:1.1rem;">Teklif Fiyatı</small>`
                : `<span class="cart__price">₺${priceStr}</span>`;

            const quantityHtml = isOverride
                ? `<div class="quantity__box">
                        <label><input type="number" class="quantity__number" value="${quantity}" readonly disabled /></label>
                   </div>`
                : `<div class="quantity__box">
            ...
            tr.innerHTML = `
                ...
                <td class="cart__table--body__list">${priceLabelHtml}</td>
                <td class="cart__table--body__list">${quantityHtml}</td>
    ...
    renderCartItems(cart) {
        ...
        items.forEach(item => {
            ...
            const quantityBoxHtml = isOverride
                ? `<span style="font-size:1.3rem;margin-right:10px;">${quantity} Adet</span>`
            ...
            const priceHtml = isOverride
                ? `<span class="minicart__current--price">₺${priceStr}</span><br><small style="color:var(--secondary-color);font-size:1.1rem;">Teklif Fiyatı</small>`
