...
namespace Otomar.WebApi.Endpoints
{
    public class CartEndpoints : ICarterModule
    {
        public void AddRoutes(IEndpointRouteBuilder app)
        {
            ...
            // Liste sorgu cevaplarından toplu sepet oluştur
            group.MapPost("/from-list-search/{requestNo}", async (string requestNo, [FromServices] ICartService cartService, CancellationToken cancellationToken) =>
            {
                var result = await cartService.AddToCartFromListSearchAsync(requestNo, cancellationToken);
                return result.ToGenericResult();
            })
            .WithName("AddToCartFromListSearch")
